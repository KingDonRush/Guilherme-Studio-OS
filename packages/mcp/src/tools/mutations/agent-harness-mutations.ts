...
export function registerAgentHarnessMutationTools(server: McpServer, root: string): void {
  ...
  server.tool(
    "studio_build_context_pack",
    {
      ...
      work_type: z.enum(["development"]).optional(),
      method_lens_answers: z.record(z.string(), z.string()).default({}),
      method_lens_not_material: z.array(z.string()).default([]),
